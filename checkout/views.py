...
def cache_checkout_data(request):
    try:
        ...
        # stripe secret key to modify payment intent
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        # prefill address details if stored in profile
...
def checkout_success(request, order_number):
    ...
    messages.success(request, f'Order successful! Your order number is \
        {order_number}. Conformation email will be sent to \
            {order.email}.')

    if request.user.is_authenticated:
        profile = UserProfile.objects.get(user=request.user)
        # Attach the user profile to order
    ...
    # delete user shopping bag from session
